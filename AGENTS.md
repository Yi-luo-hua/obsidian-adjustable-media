...
1. 永远不改写媒体嵌入文本本身，只改注释里的设置、把嵌入整段原样搬运，或者把用户在文字栏输入框里输入的文字写回这一栏自己的行（写入前确认块仍能原样读回，见 docs/DESIGN.md 第 3 节第 8 条）。
